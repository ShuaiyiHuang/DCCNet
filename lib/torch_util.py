...
def save_checkpoint(state, is_best, file, save_all_epochs=False):
    ...
    if save_all_epochs:
        ...
        if is_best:
            shutil.copyfile(join(model_dir,str(state['epoch'])+'_' + model_fn), join(model_dir,'best_' +'epoch'+str(state['epoch'])+ model_fn))
    else:
        if is_best:
            torch.save(state, file)
            shutil.copyfile(file, join(model_dir,'best_' +'dccnet'+'.pth.tar'))
